...
import { Button, Form, Input, Popconfirm, Space, Table, Typography, message, Upload } from "antd";
import React, { useEffect, useState } from "react";
import Gap from "../../components/gap/gap";
import { uploaderConfig } from "../../config/uploader-config";
import { useSingleUploader } from "../../hooks/useSingleUploader";
import { INITIAL_TABLE_DATA } from "../crudContoh/constants";
...
const FormHasura = () => {
  ...
  // Get Data
  const { data: usersData, loading: isUsersLoading, error: usersError } = useQuery(GET_USERS);
  ...
  const TABLE_COLUMNS = [
    {
      ...
      dataIndex: "avatar",
      key: "avatar",
      render: (_, record, index) => <img src={record.avatar} alt={`avatar-${index}`} style={{ height: "35px" }} />,
    },
    {
      title: "First Name",
      dataIndex: "firstName",
    ...
    <>
      ...
      >
        ...
        <Form.Item label="Avatar">
          ...
          >
            <Button icon={<UploadOutlined />} type={!avatar ? "dashed" : "default"}>
        ...
        {isEdit ? (
          <Space>
            <Button type="primary" htmlType="submit" loading={loadingUpdateUser}>
              Save
            </Button>
      ...
      {/* Table */}
      <Table rowKey="uuid" columns={TABLE_COLUMNS} dataSource={usersData?.users} loading={isUsersLoading || loadingDelete} />
    </>
  );
};
export default FormHasura;
